...
for(cross_i in 1:cross_num){
  ...
  for(file_i in file_index[-cross_i]){
    for(file_name_i in file_name_pattern){
      main_data.train.file = paste0("segmented/", file_prefix, divider, "-", file_name_i, file_i, ".txt")
      if(!exists("main_data.train")){
  ...
  for(file_name_i in file_name_pattern){
    main_data.test.file = paste0("segmented/", file_prefix, divider, "-", file_name_i, cross_i, ".txt")
    if(!exists("main_data.test")){
...
matrix.sens.spec = confusionMatrix(performance_data[,1], performance_data[,2])
