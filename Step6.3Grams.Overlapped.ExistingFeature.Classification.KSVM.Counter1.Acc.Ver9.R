...
k_value = 5
cross_num=5
feature_count.init = 1800
feature_count = 1800
feature_count.max = 1900
feature_increment = 1
...
main_data.prediction.filename = "main_data.prediction.1inc.ksvm.ver2.1.csv"
main_data.class.filename = "main_data.class.1inc.ksvm.ver2.1.csv"
features.existing.filename = "kknn-features-rangking-3grams-overlapped.50inc.accuracy.csv"

#baca features
features.existing = read.csv(paste0("3grams/result/", features.existing.filename), stringsAsFactors = FALSE)

print(paste(Sys.time(), "3Grams & All Overlapped Features using existing feature - Step: 1 - 1800-1900 - KSVM"))
...
file_prefix = paste0("3grams-overlapped-")
...
for(cross_i in 1:cross_num){
  ...
  for(file_j in file_index[-cross_i]){
    for(file_name_i in file_name_pattern){
      main_data.train.file = paste0("3grams/overlapped/", file_prefix, file_name_i, file_j, ".csv")
      if(!exists("main_data.train")){
  ...
  for(file_name_i in file_name_pattern){
    main_data.test.file = paste0("3grams/overlapped/", file_prefix, file_name_i, cross_i, ".csv")
    if(!exists("main_data.test")){
...
write.csv(main_data.prediction, paste0("3grams/result/", main_data.prediction.filename), row.names = FALSE)
write.csv(main_data.class, paste0("3grams/result/", main_data.class.filename), row.names = FALSE)
...
write.csv(main_data.perf, paste0("3grams/result/ksvm-3grams-overlapped.1inc.accuracy.ver2.1.csv"), row.names = FALSE, quote = FALSE)


#colnames(main_data.ranking) = paste0("V", c(1:ncol(main_data.ranking)))
#write.csv(main_data.ranking, paste0("3grams/result/kknn-features-rangking-3grams-overlapped.10inc.accuracy.csv"), row.names = FALSE, quote = FALSE)