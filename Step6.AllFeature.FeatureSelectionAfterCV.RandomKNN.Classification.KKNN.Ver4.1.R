...
file_prefix = paste0("ctdd-ctdc-ctdt-aac-divider-ALL-")
...
for(cross_i in 1:cross_num){
  ...
  for(file_j in file_index[-cross_i]){
    for(file_name_i in file_name_pattern){
      main_data.train.file = paste0("combination-all/", file_prefix, file_name_i, file_j, ".txt")
      if(!exists("main_data.train")){
  ...
  for(file_name_i in file_name_pattern){
    main_data.test.file = paste0("combination-all/", file_prefix, file_name_i, cross_i, ".txt")
    if(!exists("main_data.test")){
  ...
  result.rf <- rknn(main_data.train[,-ncol(main_data.train)], main_data.test[,-ncol(main_data.test)], main_data.train[,ncol(main_data.train)], k = 5)
  result.rf.features = result.rf$features
  ...
  while(feature_count < feature_count.max){
    ...
    classification_model <- rknn(main_data.train.temp[,-ncol(main_data.train.temp)], main_data.test.temp[,-ncol(main_data.test.temp)], main_data.train.temp[,ncol(main_data.train.temp)], k = 5)
    
    predict_result <- classification_model$pred
...
write.csv(main_data.perf, paste0("result/kknn-cv-allcombination.fs.divider-ALL-10inc.rknn.csv"), row.names = FALSE, quote = FALSE)


colnames(main_data.ranking) = paste0("V", c(1:ncol(main_data.ranking)))
write.csv(main_data.ranking, paste0("combination-all/kknn-10step-features-rangking.rknn.csv"), row.names = FALSE, quote = FALSE)