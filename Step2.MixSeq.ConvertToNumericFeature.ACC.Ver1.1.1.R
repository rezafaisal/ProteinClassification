...
main_data.files = cbind(c("er1.txt","er2.txt","er3.txt","er4.txt","er5.txt"))
  ...
  for(divider_i in dividers){
    ...
    for(i in 1:nrow(main_data)){
      ...
      tryCatch({
        ...
        assign("num_feature", cbind(num_feature.test))
    ...
    write.csv(main_data.num, paste0("segmented/mix-ctdd-divider-",divider_i,"-",file_i), row.names = FALSE, quote = FALSE)
    #=================================================================
