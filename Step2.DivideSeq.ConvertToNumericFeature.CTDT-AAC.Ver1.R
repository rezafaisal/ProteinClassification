  ...
  for(divider_i in dividers){
    ...
    for(i in 1:nrow(main_data)){
      ...
      tryCatch({
        ...
        datum.num = cbind(t(num_feature_ctdt), t(num_feature_aac), class)
    ...
    write.csv(main_data.num, paste0("combination/ctdt-aac-divider-",divider_i,"-",file_i), row.names = FALSE, quote = FALSE)
    #=================================================================
