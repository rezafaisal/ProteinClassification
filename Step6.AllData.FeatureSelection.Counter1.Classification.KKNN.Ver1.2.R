...
for(divider_i in dividers){
  ...
  while(feature_count <= feature_count.max){
    ...
    #print(dim(main_data.train))
    #print(dim(main_data.test))
...
write.csv(main_data.perf, paste0("result/kknn-130-210-combination.fs.divider-", divider_j,".csv"), row.names = FALSE, quote = FALSE)
