...
main_data.prediction.filename = "main_data.prediction.3grams-each.csv"
main_data.class.filename = "main_data.class.3grams-each.csv"


print(paste(Sys.time(), "word2vec feature - each"))
...
file_prefix = paste0("numerical_each.")
...
for(cross_i in 1:cross_num){
  ...
  for(file_j in file_index[-cross_i]){
    for(file_name_i in file_name_pattern){
      main_data.train.file = paste0("3grams/numerical_each/", file_prefix, file_name_i, file_j, ".txt")
      if(!exists("main_data.train")){
  ...
  for(file_name_i in file_name_pattern){
    main_data.test.file = paste0("3grams/numerical_each/", file_prefix, file_name_i, cross_i, ".txt")
    if(!exists("main_data.test")){
  ...
  #classification
  classification_model = kknn(class~., main_data.train, main_data.test[,-ncol(main_data.test)], k = k_value, kernel = "triangular")
