  ...
  for(divider_i in dividers){
    ...
    for(i in 1:nrow(main_data)){
      ...
      tryCatch({
        ...
        num_feature.test = extractPAAC(seq, lambda = 1)
        ...
        for(j in 1:divider_i){
          ...
          if(!exists("num_feature")){
            assign("num_feature", cbind(extractPAAC(seq.part, lambda = 1)))
          } else {
            num_feature = rbind(num_feature, cbind(extractPAAC(seq.part, lambda = 1)))
          }
        }
    ...
    write.csv(main_data.num, paste0("segmented/paac-divider-",divider_i,"-",file_i), row.names = FALSE, quote = FALSE)
    #=================================================================
