...
k_value = 5
cross_num=5
feature_count.init = 570
feature_count = 570
feature_count.max = 610
feature_increment = 1
...
#print(paste("Overlapped Step: 50"))
print(paste(Sys.time(), "ALL-overlapped Step: 1 570-610 features, random knn"))
...
file_prefix = paste0("ALL-overlapped-")
...
for(cross_i in 1:cross_num){
  ...
  for(file_j in file_index[-cross_i]){
    for(file_name_i in file_name_pattern){
      main_data.train.file = paste0("overlapped/result/", file_prefix, file_name_i, file_j, ".csv")
      if(!exists("main_data.train")){
  ...
  for(file_name_i in file_name_pattern){
    main_data.test.file = paste0("overlapped/result/", file_prefix, file_name_i, cross_i, ".csv")
    if(!exists("main_data.test")){
...
write.csv(main_data.perf, paste0("result/overlapped/kknn-cv-overlapped.random-knn.1inc.570-610.csv"), row.names = FALSE, quote = FALSE)


colnames(main_data.ranking) = paste0("V", c(1:ncol(main_data.ranking)))
write.csv(main_data.ranking, paste0("result/overlapped/kknn-features-rangking-overlapped.random-knn.1inc.570-610.csv"), row.names = FALSE, quote = FALSE)